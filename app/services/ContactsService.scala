...
class ContactsService {
  ...
  def contactsToRadioOptions(
    removeContact: Option[ContactOptions]
  )(implicit answers: UserAnswers): Seq[(ContactOptions, String)] =
    getRequiredContacts(answers)
      .filterNot(_.isEmpty)
      .zipWithIndex
      .map { case (contact, index) =>
  ...
  def getContactInternationalAddress(
    contactOption: ContactOptions
  )(implicit answers: UserAnswers): Option[InternationalAddress] =
    contactOption match {
      ...
      case _                                    =>
        throw new IllegalArgumentException("ContactsService.getContactInternationalAddress: Illegal Contact Option")
    }
