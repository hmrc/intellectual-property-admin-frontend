  ...
  def apply(implicit messages: Messages): Form[WhoIsSecondaryLegalContact] = Form(
    mapping(
      "companyName" -> text("whoIsSecondaryLegalContact.error.companyName.required")
        ...
        .verifying(
          regexpDynamic(regexXSSNoAmpersand, errorKeyXSSNoAmpersand, "whoIsSecondaryLegalContact.companyName.label")
        ),
    ...
    )(WhoIsSecondaryLegalContact.apply)((w: WhoIsSecondaryLegalContact) =>
      Some(w.companyName, w.contactName, w.contactTelephone, w.contactEmail)
    )
  )
}
