...
class SelectTechnicalContactController @Inject() (
              ...
              Future.successful(BadRequest(view(formWithErrors, mode, afaId, companyName, contacts))),
            {
              ...
              case _                                              =>
                throw new IllegalArgumentException(
                  "SelectTechnicalContactController.onSubmit(). unrecognised contact option "
                )
            }
          )
      }
  ...
  private def updateUserAnswersUkAddress(
    ...
    for {
      ...
      updatedAnswersWithTechUkBased <-
        Future.fromTry(updatedAnswersWithTechDetails.set(IsTechnicalContactUkBasedPage, contactIsUKBased.get))
      updatedAnswersWithUKAddress   <-
        Future.fromTry(updatedAnswersWithTechUkBased.set(TechnicalContactUkAddressPage, contactUKAddress.get))
  ...
  private def updateUserAnswersInternationalAddress(
    ...
    for {
      ...
      updatedAnswersWithTechUkBased <-
        Future.fromTry(updatedAnswersWithTechDetails.set(IsTechnicalContactUkBasedPage, contactIsUKBased.get))
      updatedAnswersWithTechAddress <-
        Future.fromTry(
          updatedAnswersWithTechUkBased.set(TechnicalContactInternationalAddressPage, contactInternationalAddress.get)
        )
