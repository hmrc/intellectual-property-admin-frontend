...
class PermissionToDestroySmallConsignmentsController @Inject()(
                                                                ...
                                                                formProvider: PermissionToDestroySmallConsignmentsFormProvider,
                                                                val controllerComponents: MessagesControllerComponents,
                                                                view: PermissionToDestroySmallConsignmentsView
  ...
  def onPageLoad(mode: Mode, afaId: AfaId): Action[AnyContent] = (identify andThen getLock(afaId) andThen getData(afaId) andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(PermissionToDestroySmallConsignmentsPage) match {
        case None => form
  ...
  def onSubmit(mode: Mode, afaId: AfaId): Action[AnyContent] = (identify andThen getLock(afaId) andThen getData(afaId) andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value => {
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(PermissionToDestroySmallConsignmentsPage, value))
            _              <- afaService.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(PermissionToDestroySmallConsignmentsPage, mode, updatedAnswers))
        }
      )
  }
}
