...
class ContactsServiceSpec extends AnyFreeSpec with Matchers with AfaGenerators with TryValues with OptionValues {
  ...
  implicit val userAnswers: UserAnswers = UserAnswers(arbitraryGbAfaIdThreeDigit().sample.value)
    ...
    .set(CompanyApplyingUkAddressPage, ukAddress).success.value
    ...
    .set(WhoIsSecondaryTechnicalContactPage, secondaryTechnicalContactDetails).success.value
  ...
  "ContactsService" - {
    "getRepresentativeContact" - {
      "must return None when contact has missing values" in {
        implicit val implicitUserAnswers: UserAnswers = userAnswers.remove(RepresentativeContactUkAddressPage).success.value
    ...
    "getApplicantLegalContact" - {
      "must return None when contact has missing values" in {
        implicit val implicitUserAnswers: UserAnswers = userAnswers.remove(ApplicantLegalContactUkAddressPage).success.value
      ...
      "must return representative contact as legal when user answers yes to rep same as legal" in {
        ...
        service.getLegalContactDetails(implicitUserAnswers) mustBe
          Some(Contact(representativeDetails.companyName, representativeDetails.contactName,
            representativeDetails.phone, None, representativeDetails.email, ukAddress))
      }
    }
    ...
    "getSecondaryLegalContact" - {
      ...
      "must return None when contact has missing values" in {
        implicit val implicitUserAnswers: UserAnswers = userAnswers.remove(ApplicantSecondaryLegalContactUkAddressPage).success.value
    ...
    "getSecondaryTechnicalContact" - {
      ...
      "must return None when contact has missing values" in {
        implicit val implicitUserAnswers: UserAnswers = userAnswers.remove(SecondaryTechnicalContactUkAddressPage).success.value
    ...
    "contactsToRadioOptions" - {
      implicit val isLegalPage: Boolean = false
        ...
        "when only rep contact is filled must return only a rep contact name with an index" in {
          ...
          service.contactsToRadioOptions(None)(implicitUserAnswers) mustBe Seq((ContactOptions.RepresentativeContact, representativeDetails.contactName))
        }
        ...
        "when only legal contact is filled must return only a legal contact name with an index" in {
          ...
          service.contactsToRadioOptions(None)(implicitUserAnswers) mustBe Seq((ContactOptions.RepresentativeContact, applicantLegalDetails.name))
        }
    ...
    "getContactIsUkBased must return the correct values for each contact option" in {
      ...
      assertThrows[IllegalArgumentException](service.getContactIsUKBased(ContactOptions.SomeoneElse)(implicitUserAnswers))
    }
    ...
    "getContactUKAddress must return the correct values for each contact option" in {
      ...
      implicit val implicitUserAnswers: UserAnswers = userAnswers
        .set(ApplicantLegalContactUkAddressPage, ukAddress2).success.value
        .set(ApplicantSecondaryLegalContactUkAddressPage, ukAddress3).success.value
      ...
      assertThrows[IllegalArgumentException](service.getContactUKAddress(ContactOptions.SomeoneElse)(implicitUserAnswers))
    }
    ...
    "getContactInternationalAddress must return the correct values for each contact option" in {
      ...
      implicit val implicitUserAnswers: UserAnswers = userAnswers
        .set(RepresentativeContactInternationalAddressPage, internationalAddress).success.value
        .set(ApplicantLegalContactInternationalAddressPage, internationalAddress2).success.value
        .set(ApplicantSecondaryLegalContactInternationalAddressPage, internationalAddress3).success.value

      service.getContactInternationalAddress(ContactOptions.RepresentativeContact)(implicitUserAnswers) mustBe Some(internationalAddress)
      service.getContactInternationalAddress(ContactOptions.LegalContact)(implicitUserAnswers) mustBe Some(internationalAddress2)
      service.getContactInternationalAddress(ContactOptions.SecondaryLegalContact)(implicitUserAnswers) mustBe Some(internationalAddress3)
      assertThrows[IllegalArgumentException](service.getContactInternationalAddress(ContactOptions.SomeoneElse)(implicitUserAnswers))
    }

    "getContact must return the correct values for each contact option" in {
      val applicantLegalContactDetails: ApplicantLegalContact = ApplicantLegalContact("legalCompany", "legalName", "legalTelephone", None, "legalEmail")
      val applicantSecondaryLegalContactDetails: WhoIsSecondaryLegalContact = WhoIsSecondaryLegalContact("legalCompany", "legalName", "legalTelephone", "legalEmail")

      implicit val implicitUserAnswers: UserAnswers = userAnswers
        .set(ApplicantLegalContactPage, applicantLegalContactDetails).success.value
        .set(WhoIsSecondaryLegalContactPage, applicantSecondaryLegalContactDetails).success.value

      service.getContact(ContactOptions.RepresentativeContact)(implicitUserAnswers) mustBe
        TechnicalContact(representativeDetails.companyName, representativeDetails.contactName, representativeDetails.phone, representativeDetails.email )
      service.getContact(ContactOptions.LegalContact)(implicitUserAnswers) mustBe
        TechnicalContact(applicantLegalContactDetails.companyName, applicantLegalContactDetails.name, applicantLegalContactDetails.telephone, applicantLegalContactDetails.email)
    ...
    "convertContactToTechnical" - {
      ...
      "return a technicalContact when defined" in {
        service.convertContactToTechnical(Some(representativeContactAsLegal)) mustBe
          TechnicalContact(representativeContact.companyName, representativeContact.contactName, representativeContact.phone, representativeContact.email )
      }
    }
