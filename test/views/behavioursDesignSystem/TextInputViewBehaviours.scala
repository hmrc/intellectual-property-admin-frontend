...
trait TextInputViewBehaviours[A] extends ViewBehaviours {
  ...
  def pageWithTextInputs(form: Form[A], createView: Form[A] => HtmlFormat.Appendable, messageKeyPrefix: String,
                         expectedFormAction: String, textInputIds: Seq[String], argsUsedInBrowserTitle: Boolean = false,
        ...
        "show an error prefix in the browser title" in {
          ...
          if (argsUsedInBrowserTitle) {
            assertEqualsValue(doc, "title",
              s"""${messages("error.browser.title.prefix")} ${messages(s"$messageKeyPrefix.title", titleArgs: _*)} - ${messages("site.service_name")}""")
          } else {
            assertEqualsValue(doc, "title",
              s"""${messages("error.browser.title.prefix")} ${messages(s"$messageKeyPrefix.title")} - ${messages("site.service_name")}""")
          }
        }
      }
  ...
  def pageWithTextArea(form: Form[A], createView: Form[A] => HtmlFormat.Appendable, messageKeyPrefix: String,
                       expectedFormAction: String, textInputIds: Seq[String], argsUsedInBrowserTitle: Boolean = false,
                       titleArgs: Seq[String] = Seq.empty,
                      ): Unit = {
