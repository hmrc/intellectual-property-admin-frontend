...
class IsExOfficioControllerSpec extends SpecBase with MockitoSugar with LockAfaChecks {

  def onwardRoute: Call = Call("GET", "/foo")

  val afaId: AfaId = userAnswersId

  val formProvider = new IsExOfficioFormProvider()
  private def form = formProvider()

  lazy val isExOfficioRoute: String = routes.IsExOfficioController.onPageLoad(NormalMode, afaId).url
  ...
  "IsExOfficio Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(IsExOfficioPage, true).success.value
