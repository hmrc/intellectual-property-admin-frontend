...
class RichJsValueSpec
  ...
  "set" - {
    ...
    "must return an error if the path is empty" in {
      ...
      value.set(JsPath, Json.obj()) shouldBe JsError("path cannot be empty")
    }
    ...
    "must set a value on a JsObject" in {
      ...
      forAll(gen) { case (originalKey, originalValue, pathKey, newValue) =>
        ...
        value.set(path, JsString(newValue)) shouldBe JsSuccess(
    ...
    "must set a nested value on a JsObject" in {
      ...
      value.set(path, JsString("baz")).asOpt.value shouldBe Json.obj(
        "foo" -> Json.obj(
          "bar" -> "baz"
        )
      )
    ...
    "must add a value to an empty JsArray" in {
      ...
      forAll(nonEmptyAlphaStr) { newValue =>
        ...
        value.set(path, JsString(newValue)) shouldBe JsSuccess(Json.arr(newValue))
      }
    }
    ...
    "must add a value to the end of a JsArray" in {
      ...
      forAll(nonEmptyAlphaStr, nonEmptyAlphaStr) { (oldValue, newValue) =>
        ...
        value.set(path, JsString(newValue)) shouldBe JsSuccess(Json.arr(oldValue, newValue))
      }
    }
    ...
    "must change a value in an existing JsArray" in {
      ...
      forAll(nonEmptyAlphaStr, nonEmptyAlphaStr, nonEmptyAlphaStr) { (firstValue, secondValue, newValue) =>
        ...
        value.set(path, JsString(newValue)) shouldBe JsSuccess(Json.arr(newValue, secondValue))
      }
    }
    ...
    "must set a nested value on a JsArray" in {
      ...
      value.set(path, JsString("bar")).asOpt.value shouldBe Json.arr(Json.arr("bar"))
    }
    ...
    "must change the value of an existing key" in {
      ...
      forAll(gen) { case (pathKey, originalValue, newValue) =>
        ...
        value.set(path, JsString(newValue)) shouldBe JsSuccess(Json.obj(pathKey -> newValue))
      }
    }
    ...
    "must return an error when trying to set a key on a non-JsObject" in {
      ...
      value.set(path, JsString("bar")) shouldBe JsError(s"cannot set a key on $value")
    }
    ...
    "must return an error when trying to set an index on a non-JsArray" in {
      ...
      value.set(path, JsString("bar")) shouldBe JsError(s"cannot set an index on $value")
    }
    ...
    "must return an error when trying to set an index other than zero on an empty array" in {
      ...
      value.set(path, JsString("bar")) shouldBe JsError("array index out of bounds: 1, []")
    }
    ...
    "must return an error when trying to set an index out of bounds" in {
      ...
      value.set(path, JsString("fork")) shouldBe JsError("array index out of bounds: 3, [\"bar\",\"baz\"]")
    }
    ...
    "must set into an array which does not exist" in {
      ...
      value.set(path, JsString("bar")) shouldBe JsSuccess(
        Json.obj(
          "foo" -> Json.arr("bar")
        )
      )
    ...
    "must set into an object which does not exist" in {
      ...
      value.set(path, JsString("baz")) shouldBe JsSuccess(
        Json.obj(
          "foo" -> Json.obj(
            "bar" -> "baz"
          )
  ...
  "remove" - {
    ...
    "must return an error if the path does not contain a value" in {
      ...
      forAll(gen) { case (originalKey, originalValue, pathKey) =>
        ...
        value.remove(path) shouldBe JsError("cannot find value at path")
    ...
    "must remove a value given an index node and return the new object for one array" in {
      ...
      forAll(gen) { case (key: String, values: List[String], indexToRemove: Int) =>
        ...
        removed shouldBe JsSuccess(expectedOutcome)
      }
    }
    ...
    "remove a value from one of many arrays" in {
      ...
      input.remove(path) shouldBe JsSuccess(
